...
```{r}
context4 <- "Sixteen business reported retaining 500 jobs with their loans, the highest amount disclosed by federal regulators."
```
...
cities <- cities %>% mutate(
  NoDataClause = case_when(
      Jobs < 1 & blanks > 0 ~ paste("Of them, ", apstylenumber(Loans), " ", Localclauseverb, " from ", ProperCity, ". However, the business provided no information about the number of jobs preserved.", sep=""), 
      blanks != Loans ~ paste("Of them, ", apstylenumber(Loans), " ", Localclauseverb, " from ", ProperCity, " and those businesses claimed in their loan paperwork to have preserved ", apstylenumber(Jobs), " jobs with that money.", " ", Blankclause, sep="")
)
)
```
